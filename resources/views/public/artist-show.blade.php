...
<x-app-layout>
    <div class="min-h-screen bg-[linear-gradient(180deg,#fff8ef_0%,#fffdf8_42%,#eef7f2_100%)] text-[#181818]">
                    ...
                    <div class="max-w-3xl">
                        ...
                        <div class="mt-5 flex flex-wrap gap-2">
                            @foreach ($socialLinks as $label => $url)
                                <a href="{{ $url }}" target="_blank" rel="noreferrer" class="rounded-full border border-black/10 bg-white px-4 py-2 text-sm font-semibold text-[#181818] shadow-sm">
                                    {{ $label }}
                                </a>
                            @endforeach
                        </div>
                        ...
                        @auth
                            <form method="POST" action="{{ $isFollowingArtist ? route('artists.unfollow', $artist) : route('artists.follow', $artist) }}">
                                ...
                                @if ($isFollowingArtist)
                                    @method('DELETE')
                                @endif
                                <button type="submit" class="rounded-full bg-[#181818] px-5 py-3 text-sm font-semibold text-white shadow-[0_16px_34px_rgba(0,0,0,0.14)]">
                                    {{ $isFollowingArtist ? 'Ne plus suivre' : 'Suivre cet artiste' }}
                                </button>
                            </form>
                        @else
                            <a href="{{ route('login') }}" class="rounded-full bg-[#181818] px-5 py-3 text-sm font-semibold text-white shadow-[0_16px_34px_rgba(0,0,0,0.14)]">
        ...
        <section class="mx-auto max-w-6xl px-6 pb-6">
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div>
                    ...
                    <p class="mt-2 text-sm text-[#525252]">Choisis une visite pour entrer dans l'univers de l'artiste.</p>
                </div>
            ...
            <div class="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                @forelse ($artist->portfolios as $portfolio)
                    ...
                    <a href="{{ route('artist-portfolios.show', $portfolio) }}" class="group overflow-hidden rounded-[1.6rem] border border-black/10 bg-white shadow-[0_18px_52px_rgba(0,0,0,0.07)] transition hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(239,71,111,0.14)]">
                        <div class="relative h-56 bg-[#181818]">
                            ...
                            <div class="absolute bottom-4 left-4 right-4 text-white">
                                <p class="text-xs font-semibold uppercase tracking-[0.24em] text-white/60">{{ $portfolio->items->count() }} scene(s)</p>
                                <h3 class="mt-1 text-xl font-semibold">{{ $portfolio->title }}</h3>
                            </div>
                        </div>
                        <div class="p-5">
                            <p class="text-sm leading-6 text-[#525252]">
                                {{ $portfolio->description ?: 'Visite immersive composee par l artiste.' }}
                            </p>
                            <span class="mt-4 inline-flex rounded-full bg-[#181818] px-4 py-2 text-xs font-semibold text-white">
                                Voir ce portfolio
                            </span>
                        </div>
                    </a>
                            ...
                            @auth
                                <form method="POST" action="{{ in_array($illustration->id, $favoriteIllustrationIds, true) ? route('illustrations.unfavorite', $illustration) : route('illustrations.favorite', $illustration) }}" class="mt-3">
                                    ...
                                    @if (in_array($illustration->id, $favoriteIllustrationIds, true))
                                        @method('DELETE')
                                    @endif
                                    <button type="submit" class="rounded-full border border-black/10 bg-[#f6f6f6] px-4 py-2 text-xs font-semibold text-[#181818]">
