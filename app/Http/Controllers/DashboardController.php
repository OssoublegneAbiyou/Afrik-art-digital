...
class DashboardController extends Controller
{
    public function __invoke(): View
    {
        ...
        $portfolios = $artist->portfolios()->withCount('items')->latest()->get();
        ...
        $dashboardProps = [
            ...
            'portfolioUrl' => route('artist-portfolios.create'),
            'portfolios' => $portfolios->map(function ($portfolio) {
                return [
                    'id' => $portfolio->id,
                    'title' => $portfolio->title,
                    'description' => $portfolio->description,
                    'itemsCount' => $portfolio->items_count,
                    'showUrl' => route('artist-portfolios.show', $portfolio),
                    'editUrl' => route('artist-portfolios.edit', $portfolio),
                ];
            })->values(),
            'csrfToken' => csrf_token(),
            'quotaError' => $errors->first('quota'),
            'storageError' => $errors->first('storage'),
            'uploadError' => $errors->first('image') ?: $errors->first('title'),
            'successMessage' => session('success'),
            'social' => [
